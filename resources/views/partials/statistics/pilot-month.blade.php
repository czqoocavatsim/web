...
<h4 class="font-weight-bold blue-text mb-4">{{ \Carbon\Carbon::now()->format('F') }} Top Pilots</h4>
    <ul class="list-unstyled">
    @php $index = 1; @endphp
        @foreach ($topPilot as $tp)
            <li class="mb-1">
                            ...
                            @else
                                <img src="{{asset('assets/resources/media/img/brand/sqr/ZQO_SQ_TSPBLUE.png')}}" style="height: 35px; !important; width: 35px !important; margin-left: 10px; margin-right: 5px; margin-bottom: 3px; border-radius: 50%;">
                                    <div class="d-flex flex-column ml-2">
                                        ...
                                        <p>{{$tp->current}} @if($tp->current == 1)flight @else flight's @endif flown in {{ \Carbon\Carbon::now()->format('F') }}</p>
                                    </div>
                            @endif
                        </span>
                    </p>
